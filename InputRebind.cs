...
namespace PvZA11y
{
    public class InputRebind
    {
        ...
        RequiredInput[] requiredInputs = new RequiredInput[]
        {
            ...
            new RequiredInput(){intent = InputIntent.Info3, description = "Say current sun count, say number of needy plants in zen garden, spin slots in SlotMachine minigames, say trophy count on gamemode selection screens, select/deselect imitater plant in plant picker."},
            new RequiredInput(){intent = InputIntent.Info4, description = "Say level progress, play audio cue if the current row has a lawnmower or a brain in iZombie levels, click snail in Zen Garden."},
